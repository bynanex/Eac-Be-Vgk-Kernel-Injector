...
{
    const auto snap = LAZY(CreateToolhelp32Snapshot).safe()(TH32CS_SNAPPROCESS, 0);
    ...
    if (!!LAZY(Process32First).safe()(snap, &proc_entry)) {
        do {
            if (name == (const char*)proc_entry.szExeFile) {
                found_process = true;
                break;
            }
        } while (!!LAZY(Process32Next).safe()(snap, &proc_entry));
    }

    LAZY(CloseHandle).safe()(snap);
